...
public class OrderStatisticTree {
    ...
    private void updatesize(Node x,int i){                 //input the parent of the node need to update
        Node cur = x;
        while(cur != nil){
            cur.size += i;
            cur = cur.p;
    ...
    public void selectRank(int i){
        System.out.println(selectRank(root, i));
    }

    private int selectRank(Node node, int i){
        int r= node.left.size+1;
        if(r == i){
            return node.key;
        }else if(i<r){
            return selectRank(node.left, i);
        }else{
            return selectRank(node.right,i-r);
        }
    }
    ...
    public static void main(String[] args){
        int[] nums = {3,2,1,5,7,9,10,33,22,15,17,90,45,65,18,12,55,8,-14,-25};
        OrderStatisticTree test = new OrderStatisticTree();
